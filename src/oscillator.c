...
const int blip_table_precision = 15;
...
static void render_square(oscillator_t* osc, int32_t* buffer, uint32_t len);
static void render_sawtooth(oscillator_t* osc, int32_t* buffer, uint32_t len);
static void render_triangle(oscillator_t* osc, int32_t* buffer, uint32_t len);
static void render_square_aliased(oscillator_t* osc, int32_t* buffer, uint32_t len);
static void render_pwm(oscillator_t* osc, int32_t* buffer, uint32_t len);
static void render_impulse(oscillator_t* osc, int32_t* buffer, uint32_t len);
static void render_noise(oscillator_t* osc, int32_t* buffer, uint32_t len);
...
void oscillator_render_to_buffer(oscillator_t* osc, int32_t* buffer, uint32_t len)
{
	...
	case OSCILLATOR_TYPE_SQUARE: {
	    render_square(osc, buffer, len);
	    break;
	}

	case OSCILLATOR_TYPE_SQUARE_ALIASED: {
	    render_square_aliased(osc, buffer, len);
	    break;
	}
...
static void render_square(oscillator_t* osc, int32_t* buffer, uint32_t len)
{
	...
	{0, ((int32_t)(0.25 * 65536)), (1 << 30)},
	{0, ((int32_t)(0.75 * 65536)), -(1 << 30)},
	{-1, -1, -1}
    };
    ...
    // integrate the pulse train to generate output
    const int64_t retention_rate_q29 = (int64_t)(0.9995 * (1 << 29));
    buffer[0] = (((int32_t)impulse_train_q15[0] << (29 - 15)) +
		 (int32_t)((retention_rate_q29 * osc->output_integrator_q2_29) >> 29));
    for (int i = 1; i < len; i++) {
	buffer[i] = (((int32_t)impulse_train_q15[i] << (29 - 15)) +
		     (int32_t)((retention_rate_q29 * buffer[i - 1]) >> 29));
    }
    osc->output_integrator_q2_29 = buffer[len - 1];
}
